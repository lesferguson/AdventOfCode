...
# ingest and format data as necessary for functions
def default_parsers(inputdata, file_format, base_type=str):
    """
    file_format types:
    raw: raw
    line: line separated
    ...
    if file_format == "raw":
        ...
        data = [base_type_conversion(line.strip().strip(), base_type) for line in inputdata.splitlines()]
    elif file_format == "tuple":
        data = [tuple([col for col in line.strip().split()]) for line in inputdata.splitlines()]
    elif file_format == "list":
        data = [[base_type_conversion(val, base_type) for val in line.strip().split(",")] for line in
        ...
        data = {}
        for j, line in enumerate(inputdata.splitlines()):
            for i, value in enumerate(line.strip()):
                data[(i, j)] = base_type_conversion(value, base_type)
    else:
        data = inputdata
