        ...
        for j, step in enumerate(logic):
            if type(step) == list:
                if d != 5:
                    ...
                    logic_list[i][j] = logic_list[i][j][0][0]
...
rules_dict[8] = '{}+'.format(rules_dict[42])

rules_dict[11] = "(?P<R>{}(?&R)?{})".format(rules_dict[42], rules_dict[31])
...
for message in messages:
    if regex.fullmatch(rules_dict[0], message):
        count += 1
        # print(message)
