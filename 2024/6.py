...
def solve_a(input_data):
    ...
    lab = {(i, j): v for j, line in enumerate(input_data.splitlines()) for i, v in enumerate(line)}
    ...
    while 0 <= guards_location[0] <= max_i and 0 <= guards_location[1] <= max_j:
        next_step = (guards_location[0]+directions[guards_direction][0], guards_location[1]+directions[guards_direction][1])
        if next_step not in lab:
            ...
            guards_direction = direction_order[(direction_order.index(guards_direction)+1) % len(direction_order)]
...
def solve_b(input_data):
    ...
    while 0 <= guards_location[0] <= max_i and 0 <= guards_location[1] <= max_j:
        next_step = (
        guards_location[0] + directions[guards_direction][0], guards_location[1] + directions[guards_direction][1])
        if next_step not in lab:
            break
        elif lab[next_step] == ".":
            guards_location = next_step
    ...
    for node in orig_path:
        ...
        while 0 <= guards_location[0] <= max_i and 0 <= guards_location[1] <= max_j:
            ...
            if next_step not in lab:
                ...
                guards_location = next_step
                if guards_location in visited and guards_direction == visited[guards_location]:
                    solution += 1
                    break
                visited[guards_location] = guards_direction
            else:
